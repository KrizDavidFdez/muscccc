        ...
        {lyrics.map((line, index) => {
          ...
          return (
            <motion.div
              ...
              transition={{
                // Layout spring: snappy but smooth — matches the "snap into place" feel of the video
                layout: {
                  type: 'spring',
                  damping: 30,
                  stiffness: 300,
                ...
                // Visual props: fast ease-out so blur/opacity land quickly, not sluggishly
                filter:  { duration: 0.2, ease: 'easeOut' },
                opacity: { duration: 0.2, ease: 'easeOut' },
                scale:   { duration: 0.2, ease: 'easeOut' },
              }}
              style={{ transformOrigin: 'left center' }}
              className={`group transition-colors cursor-pointer text-left w-full max-w-2xl py-1 ${
                isActive
